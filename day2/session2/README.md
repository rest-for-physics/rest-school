We can use the REST-for-Physics libraries in different environments, including `restRoot` ROOT based interactive shell, `C-macros` executed by the ROOT interface interpreter, C++ compiled executables, or `python3` scripts.

In this session we will learn how to access REST-for-Physics generated files using `python3`, although the use of this commands, or code lines, can be used in a similar way in all the previous environments mentioned.
