...
We will compile a very simple class with a data member that will allow us to test the ROOT I/O features. First we will create the class source and header files.
...
We will create a header file named `Dummy.h` containing three data members, where one of them `fTempValue` will be ignored from the I/O streamer. The class will also define getter and setter methods, and it will prototype a `Print` method to print the values stored inside the class.
...
You should have remarked now the 3 elements that identify the complete ROOT-ification of our class. The class is inheriting from `TObject`, and the class implements the `ClassImp(Dummy)` and `ClassDef(Dummy, 1)` that extends the class with additional methods that must be defined by any ROOT class.
...
We will create a directory for our project and edit a new file inside named `CMakeLists.txt`.

```
nano CMakeLists.txt
```

The first thing we need to add to our project definition is its name and the minimum `cmake` version required. `cmake` is an active evolving project that implements new features as new versions come out, and some of the predefined `cmake` functions might become obsolete.

Add the following lines to the file to force the project to be configured with at least cmake 3.0, and name the project with an arbitrary name, such as `dummy`:
...
Then we need to make sure the ROOT libraries are installed and accessible in our system

```
find_package(ROOT REQUIRED COMPONENTS RIO Net)
```

Add the following line to add new paths were header files should be found. In our case we will just add the root directory since our simple project will just have one single header file.

```
include_directories(${CMAKE_CURRENT_SOURCE_DIR})
```

On top of that, to profit from all the advantages of automatic schema evolution in ROOT we will need to generate a ROOT dictionary for our class, we can do this adding the following lines to our `CMakeLists.txt`.
...
We then define the generation of a library where we add all the classes that want to be compiled together, in our case we just got one class `Dummy.cxx` so we just add one target to be compiled within the final library file `libDummy.so`.
...
We then need to add any dependencies required to build the `Dummy` compiled class. This line will trigger the generation of the dictionary defined at `ROOT_GENERATE_DICTIONARY` which is necessary so that our class implements properly the dictionary features from ROOT.

```
target_sources(Dummy PRIVATE DummyDict.cxx)
```

We got already all the instructions required to compile our library (`libDummy.so`, `libDummy.dylib` or `libDummy.dll` depending on the architecture of our OS) which will be ready to be integrated within ROOT.
...
On sucess, we will be able to compile now our library
...
One compilation has suceeded, and before testing our new library we must define few installation instructions that will help cmake to decide where the final compiled binaries should be placed, by adding the following lines to our `CMakeLists.txt`.
...
on top of the targets that we asked cmake to generate, ROOT generates an additional `.pcm` file that needs to be placed at the same location as the target library. Thus, on top of that we need to add the following line
...
```
add_executable(Main MainDummy.cxx)
...
You may try now to recover the contents saved on the files generated by the ROOT shell commands and the executable.
